...
## Open questions
BLOCKS-EXECUTION:
- Confirm the live `teachers` table's actual column names/types for `instruments` (and whether `bio`, `education`, `credentials`, `online_available` exist at all yet) before Task 1 writes the row mapping — this plan assumes they exist per your answer but the exact shape is unverified in-repo.
